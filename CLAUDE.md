...
- 프레젠테이션 작업 (슬라이드/덱/프레젠테이션 컴포넌트) → `presentation-work` Skill 사용
...
- 프레젠테이션용 일러스트 생성 → `presentation-illustration` Skill 사용
- 비트맵 → SVG 아이소메트릭 변환 → `bitmap-to-iso` Skill 사용 (측정 강제)
- 이미지 생성 (Nano Banana) → `nano-banana` Skill 사용
- OG 이미지 생성·배치·검증 → `og-optimized` Skill 사용 (nano-banana와 연계)
