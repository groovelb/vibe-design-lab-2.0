...
- **Construct 시스템** — ■ (6×6 construct square)를 브랜드 키 비주얼로 활용. 3개 컴포넌트가 각각 다른 방식으로 ■를 사용하여 고유한 모션 시그니처를 표현.
  ...
  - ConstructBlock: native CSS wrapping + clip-path 기반 스캔 리빌. visual line 수만큼 ■ 커서가 동시에 좌→우 sweep하며 문단 마스킹 해제. 본문, 설명 등 긴 텍스트용 (`components/motion/ConstructBlock.jsx`)
