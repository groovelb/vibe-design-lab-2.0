...
- PeekScroll: 가로 스크롤 + 다음 아이템 peek 패턴 컨테이너. CSS scroll-snap 기반, 1col 아이템 + peek 노출로 스크롤 어포던스 제공. inset으로 full-bleed 정렬 (`components/container/PeekScroll.jsx`)
...
- TaxonomySection: Dictionary 페이지 핵심 인터랙티브 영역. Stats strip + sticky Part tabs + 카테고리 아코디언. Parts 1-2는 컴포넌트 테이블, Part 3는 디자인 사조 테이블, Part 4는 레퍼런스 테이블 (`components/data-display/TaxonomySection.jsx`)
- TaxonomyTree: 트리 구조 텍소노미 표시. MUI SimpleTreeView 기반, Monoline 스타일. full/preview variant (`components/data-display/TaxonomyTree.jsx`)
- TimelineBlock: 순차적 단계 타임라인. 도트 + 라인 + 챕터 콘텐츠 (`components/data-display/TimelineBlock.jsx`)
- StickyNodeExplorer: 스크롤 드리븐 노드 그래프 스토리텔링. sticky SVG 그래프(600×500) + IntersectionObserver 스크롤 스텝. 점진적 노드/엣지 활성화, edge draw-in 애니메이션, 모바일 스택 폴백 (`components/data-display/StickyNodeExplorer.jsx`)
...
- GradientText: 루핑 그라디언트 마스킹 타이포그래피. CSS background-clip + background-position 애니메이션. colors/duration/angle/variant 커스터마이즈 가능 (`components/kinetic-typography/GradientText.jsx`)
...
- ParallaxLayer: 스크롤 속도 배수 패럴럭스 래퍼. speed>1=빠르게, <1=느리게. Framer Motion useScroll + useTransform (`components/scroll/ParallaxLayer.jsx`)
...
- GeometricX: 기하학적 X 마크 SVG. 뷰포트 진입 시 stroke-draw 애니메이션. IntersectionObserver + CSS transition 기반, prefers-reduced-motion 대응 (`components/motion/GeometricX.jsx`)
...
- ContextEngineAmbient: ContextEngineV2의 구조적 요소(커넥션 경로, 파티클, 스캔라인, 노드 글로우)만 추출한 앰비언트 배경. UI 텍스트/아이콘 제거, progress(0→1) 연동 opacity, radial vignette mask, xMidYMid slice. prefers-reduced-motion 대응 (`components/motion/ContextEngineAmbient.jsx`)
...
- AmbientGrainedBackground: violetGray 스케일 기반 모노톤 ambient grained gradient 배경. 5-레이어 합성 (Base → Glow → Gradient → Section Grain → Global Grain). hasScrollReactive로 스크롤 진행률에 따라 gradient stop/glow 위치 동적 변화 (CSS custom property + RAF, 리렌더링 없음). prefers-reduced-motion 대응 (`components/dynamic-color/AmbientGrainedBackground.jsx`)
...
- TechBadgeIcons: 기술 브랜드 SVG 아이콘 맵 (React.js, Storybook, Antigravity, Claude, Figma, MUI). Chip icon prop으로 사용 (`common/ui/TechBadgeIcons.jsx`)
- glassSx: 글래스 모피즘 공유 sx 스타일 (GLASS_SX, GLASS_DIVIDER_SX). 반투명 오버레이 특성상 alpha 값 사용 (`common/ui/glassSx.js`)
- FilterBar: 필터 바 (`components/templates/FilterBar.jsx`)

---

## Presentation (프레젠테이션 전용)

VDL 디자인 시스템과 토큰 분리. HD 이상 데스크탑 16:9 전용. `src/styles/themes/presentation.js` 토큰 사용.

### Shell
- PresentationProvider: 프레젠테이션 토큰 context provider (`components/presentation/PresentationProvider.jsx`)
- SlideMaster: 슬라이드 마스터 레이아웃 (패딩 + 브레드크럼 + 네비게이션 + Drawer). 키보드 ArrowLeft/Right 바인딩 (`components/presentation/SlideMaster.jsx`)

### Layout
- SlideChapterTitle: 챕터 진입 슬라이드. overline + title + summary + ToC (`components/presentation/layouts/SlideChapterTitle.jsx`)
- SlideHSplit: 수평 n등분 레이아웃 (2~4 컬럼) (`components/presentation/layouts/SlideHSplit.jsx`)
- SlideGrid: n×n 그리드 레이아웃 (`components/presentation/layouts/SlideGrid.jsx`)
- SlideMessage: 중앙 정렬 단일 메시지 (`components/presentation/layouts/SlideMessage.jsx`)

### Element
- SlideTitle: 슬라이드 h1. level 1(title)/2(subtitle) (`components/presentation/elements/SlideTitle.jsx`)
- SlideImage: 비율 지정/원본 이미지 + 캡션 (`components/presentation/elements/SlideImage.jsx`)
- SlideStorytelling: 두 문장 화살표 연결 (인과관계 시각화). vertical/horizontal (`components/presentation/elements/SlideStorytelling.jsx`)
- SlideTypoStack: 4단 위계 텍스트 블록 (title/subtitle/headline/body) (`components/presentation/elements/SlideTypoStack.jsx`)
- SlideList: 불렛/넘버 리스트. headline/body 크기 (`components/presentation/elements/SlideList.jsx`)
